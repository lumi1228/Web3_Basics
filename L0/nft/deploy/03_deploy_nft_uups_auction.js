...
module.exports = async ({getNamedAccounts, deployments}) => {
  ...
  // 通过代理部署合约
  const NftAuctionUUPSProxy = await upgrades.deployProxy(NftAuctionUUPS, [], {
    initializer: 'initialize',//调用 initialize 函数进行初始化
  });

  // 等待合约部署完成
  await NftAuctionUUPSProxy.waitForDeployment();

  const proxyAddress = await NftAuctionUUPSProxy.getAddress();
  ...
  fs.writeFileSync(storePath, JSON.stringify({
    proxyAddress,
    implAddress,
    abi:NftAuctionUUPS.interface.format('json'),
  }));
...
module.exports.tags = ['depolyNftAuctionUUPS'];